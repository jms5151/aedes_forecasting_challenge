...
seiseir_aegypti_model <- function(t, state, parameters) {
...
K_tr <- function(temp, rain, Rmax, N){
  ...
  if((rain < R0) | (rain > Rmax)){
    0.01*carrying_capacity_t(temp,29.0,0.05, N)
  }
  else {
    c <- -5.99e-03
    carrying_capacity_t(temp,29.0,0.05, N)*(c*(rain-R0)*(rain-Rmax))/(rain/4) + 0.001
  }
}
